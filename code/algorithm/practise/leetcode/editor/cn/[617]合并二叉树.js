...
var mergeTrees = function(root1, root2) {
    ...
    // 同时前序遍历
    function start(node1, node2, newNode) {
        if (node1 && node2) {
        ...
        if ((node1 && node1.left) ||(node2 && node2.left)) {
            ...
            start(node1 ? node1.left : null,node2 ? node2.left : null, newNode.left)
        }

        if ((node1 && node1.right) ||(node2 && node2.right)) {
            newNode.right = new TreeNode()
            start(node1 ? node1.right : null, node2 ? node2.right : null, newNode.right)
        }
    }
    start(root1, root2, result)
